...
namespace RFK
{
    class AnimatedText : GameObject
    {
        ...
        public AnimatedText(string text, float start_time, float end_time, Vector2 position, int fontSize, Color color, SceneHandler scene_handler)
        {
            this.text = text;
        ...
        public void draw()
        {
            int line_characters = (int) Math.Ceiling((float)(this.scene_handler.getSize().X - this.position.X) / (float) this.fontSize);
            ...
            while (sub_start < this.text.Length && sub_start < this.characters_printed)
            {
                ...
                DrawText(this.text.Substring(sub_start, text_length), (int) (this.position.X + offset.X), (int) (this.position.Y + offset.Y), this.fontSize, this.color);
                sub_start += line_characters;
                offset.Y += this.fontSize;
            }
        }
    }
}